...
This pilot analysis, will determine which of the 2 doses of deferiprone (DFP) has the greatest effect on zebrafish behaviour at 7 months of age, 1 day post injection. 
...
meta <- read_xlsx("data/pilot/2023_ip_DFOpilot_meta.xlsx") %>% 
  mutate(fish_id = as.character(fish_id), 
         genotype = factor(genotype, levels = c("het", "hom")),
         HomeTank = factor(HomeTank, levels =c(1,2)),
         sex = as.factor(sex),
         treatment = str_replace(treatment, pattern = "O", replacement = "P"), 
...
final_data <- read_csv("data/pilot/processed_data/final_output.csv") %>% 
  dplyr::select(-1) %>% 
...
I manually inspected the video tracking for fish fish 30, 32, 33, and 57 and observed some jumping of the cross from the fish, indicating this high number of turns is not real and so this fish will be omitted. 


```{r}
# define fish which had the tracking issues
fish2omit <- c(30, 32, 33, 57) 
...
Generally, a relatively equal number of fish per genotype treatment should be tested for a nice experiment. 
...
# note the full interatcion efixed effect did not workk, likely due to not enough fish per genotype. 
...
Looking at just the treatment and genotypes of interest. 
```{r, fig.height=10}
print(emmeans(glm_reps, ~ genotype * treatment * bin), type = "response") %>% 
  as_tibble() %>% 
  mutate(binforvis = case_when(
    ...
    genotreat = paste0(genotype, "_", treatment)) %>% 
  dplyr::filter(genotreat %in% c("het_0.85% saline", 
                                 "hom_0.85% saline", 
                                 "hom_dose 1 DFP",
                                 "hom_dose 2 DFP"
  )) %>% 
  ggplot(aes(x = binforvis, y = prob, colour = genotreat)) +
  geom_col(aes(fill = genotreat), 
           alpha = 0.25,
           position = position_dodge()) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL),
                position = position_dodge()) +
  geom_smooth(aes(colour = genotreat, group = genotreat))
  facet_wrap(~treatment, nrow = 1) 
...
At this stage, it is difficult to say whether the the Treatment of deferirpone is having any benefit on 6 month old naglu homozygous zebrafish.  But dose 1 looking ok. 
