...
#reflectign the `behavBatch` col in the metadata spreadsheet.
# the next few lines of code will replace the behav batch to a string of fish_ids


# make a df which contains the fish info
subject2fish <- meta %>%
...
df$head <- df$head %>%
  lapply(function(x) {
    x %>%
      mutate(X4 = case_when(
        X4 == 1 ~ subject2fish[X4,]$fish,
        X4 == 2 ~ subject2fish[X4,]$fish,
        X4 == 3 ~ subject2fish[X4,]$fish,
        X4 == 4 ~ subject2fish[X4,]$fish,
        X4 == 5 ~ subject2fish[X4,]$fish,
        X4 == 6 ~ subject2fish[X4,]$fish,
        X4 == 7 ~ subject2fish[X4,]$fish,
        X4 == 8 ~ subject2fish[X4,]$fish,
        X4 == 9 ~ subject2fish[X4,]$fish,
        X4 == 10 ~ subject2fish[X4,]$fish,
        X4 == 11 ~ subject2fish[X4,]$fish,
        X4 == 12 ~ subject2fish[X4,]$fish,
        X4 == 13 ~ subject2fish[X4,]$fish,
        X4 == 14 ~ subject2fish[X4,]$fish,
        X4 == 15 ~ subject2fish[X4,]$fish,
        TRUE ~ X4
      ))
  })
...
df[[2]] <- df[[2]] %>%
  map(~ mutate(.x, X4 = as.character(X4))) # ensure that "X4" is character because unnest() can't handle mixed data types in list
