...
file_list <- list.files(path = here("data/Full exp 2/raw_data/distance_data"),
                        pattern = "*.csv",
                        full.names = TRUE,
                        recursive = TRUE)


m# make an object containing all the raw data files.
df <-
  tibble(data_file_distances = file_list) %>%  #Import data as a tibble with nested lists
  mutate(data = map(file_list, ~ read_csv(.x )),
         ...
         data_file_distances = str_remove(data_file_distances,
                                          pattern = "/Users/karissabarthelson/Documents/2023_iron_chelation_MPSIII/data/Full exp 2/raw_data/distance_data/"))


# convert to to a tibble rather than a list
df %<>%
  unnest(data)
...
df <- df %>%
  ...
  mutate(ymazePosition = as.character(ymazePosition)) %>% # convert to character to get compatible column types to allow the join on the next step
  left_join(meta,
...
df %>%
  saveRDS("data/Full exp 2/processed_data/distanceData.rds")
...
df %>%
  ...
  ggplot(aes(x = start.time, y = total_dist, colour = treatment)) +
